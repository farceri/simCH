...
# ── Initial droplet ───────────────────────────────────────────────────────
R0           = 30.0                 # initial radius
noise_amp    = 0.1                  # amplitude of random interface perturbations
aspect_ratio = 1                    # initial aspect ratio of droplet
...
def solve_pressure(phi, K):
    """
    Solve  ∇·[K(φ) ∇p] = −S(φ)   [note the minus sign]

    From Darcy v = −K∇p  and  ∇·v = S(φ):
        ∇·(−K∇p) = S  →  ∇·(K∇p) = −S

    A positive source S inside the droplet creates a pressure maximum
    there, so v = −K∇p points outward → droplet expands.  The previous
    code had +S, which inverted the velocity and caused collapse.

    Splitting K = K̄ + δK, each Picard iteration solves:
        K̄ ∇²p^(n+1) = −S − ∇·(δK ∇p^n)
    which in Fourier space is:
        p̂^(n+1) = (Ŝ + corr_hat) / (K̄ |k|²)
    """
    S     = growth_source(phi)
    K_bar = float(np.mean(K))
    dK    = K - K_bar

    # Initial guess: K̄ ∇²p = −S  →  p̂ = Ŝ / (K̄ |k|²)
    S_hat         = fft2(S)
    S_hat[0, 0]   = 0.0
    p_hat         = S_hat / (K_bar * k2_nz)   # <-- was  -S_hat / ...
    p_hat[0, 0]   = 0.0

    for _ in range(n_p_iter):
        px, py        = gradient(p_hat)
        corr          = divergence(dK * px, dK * py)
        rhs_hat       = S_hat + fft2(corr)    # <-- was  fft2(S - corr)
        rhs_hat[0, 0] = 0.0
        p_hat         = rhs_hat / (K_bar * k2_nz)   # <-- was  -rhs_hat / ...
        p_hat[0, 0]   = 0.0
...
def make_initial_phi(seed=42, aspect=aspect_ratio):   # aspect > 1 → ellipse
    """
...
def run_simulation(r_vis_value=None, k_grow_value=None, aspect_ratio_value=None, n_steps_value=None, verbose=True):
    """
...
if __name__ == '__main__':
    ...
    r_vis_value = float(sys.argv[1])
    k_grow_value = float(sys.argv[2])
    aspect_ratio_value = float(sys.argv[3])
    n_steps_value = int(float(sys.argv[4]))
    frames, diag = run_simulation(r_vis_value, k_grow_value, aspect_ratio_value, n_steps_value)
    ...
    # ── STEP 3 & 4: compare different activity levels ─────────────────────
